...
class Orders
{
    ...
    /**
     * @ORM\ManyToOne(targetEntity=Customers::class, inversedBy="CustomersgetOrders")
     */
    private $CustomerID;
     ...
     */
    private $OrdersGetOrdersDetails;

    public function __construct()
    {
        $this->OrdersGetOrdersDetails = new ArrayCollection();
    }
     ...
     */
    public function getOrdersGetOrdersDetails(): Collection
    {
        return $this->OrdersGetOrdersDetails;
    }

    public function addOrdersGetOrdersDetail(OrderDetails $ordersGetOrdersDetail): self
    {
        if (!$this->OrdersGetOrdersDetails->contains($ordersGetOrdersDetail)) {
            $this->OrdersGetOrdersDetails[] = $ordersGetOrdersDetail;
            $ordersGetOrdersDetail->setOrderID($this);
        }

        return $this;
    }

    public function removeOrdersGetOrdersDetail(OrderDetails $ordersGetOrdersDetail): self
    {
        if ($this->OrdersGetOrdersDetails->removeElement($ordersGetOrdersDetail)) {
            // set the owning side to null (unless already changed)
            if ($ordersGetOrdersDetail->getOrderID() === $this) {
                $ordersGetOrdersDetail->setOrderID(null);
            }
        }
