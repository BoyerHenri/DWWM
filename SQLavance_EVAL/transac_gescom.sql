...
/*  
    ...
    ***************************************************
    ...
    5a Creer la table
    6. Taper la procedure
    7. Effectuer l atransaction sur cette procedure avec un call
*/
...
DELIMITER //
    ...
    BEGIN
        ...
        UPDATE employees SET emp_pos_id=2 WHERE emp_enter_date=new_emp_enter_date;
        -- emp_salary=(emp_salary*1.05)
        -- Augmentation du nouveau (Par defaut Amity 2 Nouveau 20450 14 pepininieristes pour tests )
        UPDATE employees SET emp_salary=(emp_salary*1.05) WHERE emp_enter_date=new_emp_enter_date;
        -- Mise en retraite de l'employé (TO DO METTRE 36)
        UPDATE employees SET emp_pos_id=36 WHERE emp_lastname='HANNAH' AND emp_firstname='Amity';
        -- Passer les pepiniéristes sous le joug du nouveau
        UPDATE employees SET emp_superior_id=new_emp_id WHERE emp_pos_id=14;
    END //
DELIMITER ;
-- Test
CALL mise_en_retraite();
...
-- Zone de test et d'essais NUCLEAIRES (boum)
      ...
      -- Declaration des variables afin de swapp
        ...
        -- *Remplacement*
        -- **************
        -- Augmentation du nouveau (Par defaut Amity 2 Nouveau 20450 pour tests)
        UPDATE employees SET emp_salary=(emp_salary*1.05) WHERE emp_enter_date=new_emp_enter_date;
        -- Mise en retraite de l'employé (TO DO METTRE 36)
