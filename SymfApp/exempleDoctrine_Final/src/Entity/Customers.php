...
class Customers
{
    ...
    public function addOrderRelation(Orders $orderRelation): self
    {
        if (!$this->OrderRelation->contains($orderRelation)) {
            $this->OrderRelation[] = $orderRelation;
            $orderRelation->setCustomerID($this);
        }

        return $this;
    }

    public function removeOrderRelation(Orders $orderRelation): self
    {
        if ($this->OrderRelation->removeElement($orderRelation)) {
            // set the owning side to null (unless already changed)
            if ($orderRelation->getCustomerID() === $this) {
                $orderRelation->setCustomerID(null);
            }
        }
